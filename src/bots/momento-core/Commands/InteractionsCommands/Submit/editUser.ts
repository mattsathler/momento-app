...
async function editUserProfile(ctx: IContext, interaction: ModalSubmitInteraction) {
    try {
        ...
        let newUserInfo: IEditableFields = {
            ...
            pronouns: formField.pronouns ?? null
        };
...
function fetchFormFields(interaction: ModalSubmitInteraction): IEditableFields | null {
    ...
    const pronounsField = interaction.fields.getField('pronouns_field', ComponentType.TextInput).value
    ...
    const pronouns = pronounsField.length > 0 ? pronounsField : null;

    if (!username && !name && !surname && !bio && !pronouns) { return null }
    return { username, name, surname, bio, pronouns }
}